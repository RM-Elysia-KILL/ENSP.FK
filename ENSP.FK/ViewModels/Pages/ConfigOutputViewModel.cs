...
public partial class ConfigOutputViewModel : ObservableObject, INavigationAware
{
    ...
    private async Task GenerateConfigs()
    {
        ...
        try
        {
            ...
            if (aiConfigs != null && aiConfigs.Count > 0)
            {
                ...
                // Fire-and-forget: auto-verify generated configs against requirements
                _ = Task.Run(async () =>
                {
    ...
    private async Task VerifyConfigs()
    {
        if (_session.Configs.Count == 0)
        {
            AddChatMessage("status", "没有可验证的配置 — 请先生成配置");
            return;
        }
    ...
    private async Task VerifyConfigsInternalAsync()
    {
        ...
        try
        {
            ...
            AddChatMessage("status", "正在将生成的配置返回 AI 进行验证...");
            ...
            if (result != null)
            {
                ...
                AddChatMessage("status", "✓ 验证完成");

                if (result.Contains("✓ 验证通过") && !result.Contains("✗ 存在问题"))
                    StatusMessage = "AI 验证通过 — 配置满足所有需求";
                else
                    StatusMessage = "AI 验证发现问题 — 请查看验证结果";
            }
            else
            {
                AddChatMessage("status", $"✗ 验证失败 — {_aiGenerator.LastError}");
            }
        }
        catch (Exception ex)
        {
